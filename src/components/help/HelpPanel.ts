...
function renderDoc(doc: HelpDoc) {
  ...
  if (doc.details?.length) {
    ...
    for (const d of doc.details.slice(0, 8)) { const li = document.createElement('li'); li.innerHTML = wrapHits(d, lastSearchQuery); ul.appendChild(li) }
    body.appendChild(ul)
  }
  if (doc.parts?.length) {
...
function renderIndex(body: HTMLElement) {
  const docs = listDocs().slice().sort((a, b) => a.id.localeCompare(b.id))
  ...
  for (const [group, list] of Array.from(groups.entries()).sort((a,b) => a[0].localeCompare(b[0]))) {
  ...
  try {
    ...
    if (all && (import.meta as any)?.env?.DEV) {
      ...
      const missing = Array.from(all).filter(id => !have.has(id)).sort()
      if (missing.length) {
...
function openPanelWithDoc(doc: HelpDoc) {
  try { (window as any).__smkHelpEvents = (window as any).__smkHelpEvents || []; (window as any).__smkHelpEvents.push({ t: Date.now(), event: 'help.panel_opened', data: { id: doc.id } }) } catch {}
  ensureStyles()
  if (!overlayEl) {
...
function openSearch() {
  ensureStyles()
  try { (window as any).__smkHelpEvents = (window as any).__smkHelpEvents || []; (window as any).__smkHelpEvents.push({ t: Date.now(), event: 'help.search_opened' }) } catch {}
  if (!searchEl) {
    ...
    async function renderResults2(q: string) {
      ...
      try {
        ...
        if (res.ok) { const payload = await res.json(); if (payload && payload.index) buildIndex = payload.index }
      } catch {}
      const terms = new Set<string>([query])
      Object.entries(SYNONYMS).forEach(([canon, list]) => { if (query && (query === canon || list.includes(query))) { terms.add(canon); list.forEach(t => terms.add(t)) } })
      const scored = items.map((d) => {
        const text = (d.label + ' ' + (d.summary || '') + ' ' + (d.details || []).join(' ')).toLowerCase()
        let best = 0
        terms.forEach((t) => {
          if (!t) return
          let s = 0
          if (buildIndex && buildIndex[t] && buildIndex[t].includes(d.id)) { s = 2 }
          else { const idx = text.indexOf(t); s = idx >= 0 ? (1 / (1 + idx)) + Math.min(1, t.length / Math.max(1, text.length)) : 0 }
          if (s > best) best = s
        })
        return { d, score: best }
      ...
      list.forEach((d, i) => {
        ...
        if (query) { text = wrapHits(text, query) }
        row.innerHTML = text
        row.addEventListener('mouseenter', () => { activeIndex = i; updateActive() })
        row.addEventListener('click', () => { lastSearchQuery = query; openById(d.id); closeSearch() })
    ...
    const renderResults = (q: string) => {
      ...
      const scored = items.map((d) => {
        ...
        const score = query ? (idx >= 0 ? (1 / (1 + idx)) + Math.min(1, query.length / Math.max(1, text.length)) : 0) : 0
        return { d, score }
      }).filter(x => query ? x.score > 0 : true)
      scored.sort((a,b) => b.score - a.score)
      const list = (query ? scored.map(x => x.d) : items).slice(0, 50)
      list.forEach((d, i) => {
        ...
        row.innerHTML = text
        row.addEventListener('mouseenter', () => { activeIndex = i; updateActive() })
        row.addEventListener('click', () => { openById(d.id); closeSearch() })
        results.appendChild(row)
      })
    }
    const updateActive = () => {
      const rows = results.querySelectorAll('.smk-result')
      rows.forEach((r, i) => { if (i === activeIndex) r.classList.add('active'); else r.classList.remove('active') })
    }
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') { e.preventDefault(); closeSearch(); return }
      const rows = results.querySelectorAll('.smk-result')
      if (e.key === 'ArrowDown') { activeIndex = Math.min(rows.length - 1, activeIndex + 1); updateActive(); e.preventDefault(); }
      if (e.key === 'ArrowUp') { activeIndex = Math.max(0, activeIndex - 1); updateActive(); e.preventDefault(); }
      if (e.key === 'Enter') {
        const row = rows[activeIndex] as HTMLElement | undefined
        const id = items[activeIndex]?.id
        if (row && id) { lastSearchQuery = (searchInput!.value || '').trim().toLowerCase(); try { (window as any).__smkHelpEvents?.push({ t: Date.now(), event: 'help.search_result_opened', data: { id, q: lastSearchQuery } }) } catch {}; openById(id); closeSearch(); }
      }
    }
    const onInput = () => { const q = (searchInput!.value || '').trim().toLowerCase(); try { (window as any).__smkHelpEvents?.push({ t: Date.now(), event: 'help.search_query', data: { q } }) } catch {}; renderResults2(q) }
    searchInput.addEventListener('keydown', onKey)
    searchInput.addEventListener('input', onInput)

    const refresh = () => { items = listAllDocs(); activeIndex = 0; renderResults2(searchInput!.value || '') }
...
function closeSearch() { if (searchInput) lastSearchQuery = (searchInput.value || '').trim().toLowerCase(); searchEl?.classList.remove('open') }

function setupShortcuts() {
  window.addEventListener('keydown', (e) => {
    const meta = (e.ctrlKey || e.metaKey)
    if (meta && (e.key === '/' || e.code === 'Slash')) { e.preventDefault(); if (panelEl?.classList.contains('open')) closePanel(); else openPanelWithDoc({ id:'help.index', label:'Help', summary:'Search or click a control’s ? icon to see details.' } as HelpDoc); }
...
export function openHelpPanel(id?: string) { if (id) openById(id); else openPanelWithDoc({ id:'help.index', label:'Help', summary:'Search with Cmd/Ctrl+K or click a control’s ? icon.' } as HelpDoc) }
export function closeHelpPanel() { closePanel() }
export function openHelpSearch() { openSearch() }

// Allow callers to open the panel with a provided doc (used when a help id lacks a registry entry)
export function openHelpPanelWithDoc(doc: HelpDoc) { openPanelWithDoc(doc) }
