...
const sanitizeProfilePoints = (points: any, clampT = (t: number) => clamp(t, 0, 1), clampV = (v: number) => v) => {
...
const sanitizeCurveProfile = (b: any): BladeParams['curveProfile'] | undefined => {
  ...
  const points = sanitizeProfilePoints(b.points, (t) => clamp(t, 0, 1), (v) => clamp(v, -5, 5))
...
const sanitizeWaviness = (raw: any, baseWidth: number): BladeWaviness | undefined => {
  ...
  const amplitude = clamp(typeof raw.amplitude === 'number' ? raw.amplitude : 0, 0, Math.max(0.001, baseWidth * 1.2))
  ...
  const offset = clamp(typeof raw.offset === 'number' ? raw.offset : 0, -Math.max(0.5, baseWidth), Math.max(0.5, baseWidth))
  const mode: BladeWaviness['mode'] = raw.mode === 'centerline' || raw.mode === 'both' ? raw.mode : 'width'
...
const sanitizeFullers = (raw: any, blade: BladeParams): Array<FullerSlot> | undefined => {
  ...
  for (const entry of raw) {
    ...
    const side: FullerSlot['side'] = entry.side === 'left' || entry.side === 'right' ? entry.side : 'both'
    const width = clamp(typeof entry.width === 'number' ? entry.width : (blade.baseWidth || 0.25) * 0.3, 0.005, Math.max(0.05, blade.baseWidth * 0.9))
    const offsetFromSpine = clamp(typeof entry.offsetFromSpine === 'number' ? entry.offsetFromSpine : 0, -maxOffset, maxOffset)
    const depth = clamp(typeof entry.depth === 'number' ? entry.depth : blade.fullerDepth ?? 0.02, 0, 0.2)
    const inset = clamp(typeof entry.inset === 'number' ? entry.inset : blade.fullerInset ?? depth, 0, 0.2)
    ...
    const profile: FullerSlot['profile'] = entry.profile === 'v' || entry.profile === 'flat' ? entry.profile : 'u'
...
const buildLegacyFullers = (blade: BladeParams): Array<FullerSlot> | undefined => {
  ...
  const width = blade.fullerWidth && blade.fullerWidth > 0 ? blade.fullerWidth : (blade.baseWidth || 0.25) * 0.3
  const depth = blade.fullerDepth ?? 0.02
  const inset = blade.fullerInset ?? depth
  const profile: FullerSlot['profile'] = blade.fullerProfile === 'v' || blade.fullerProfile === 'flat' ? blade.fullerProfile : 'u'
...
export function validateSwordParams(params: SwordParams): SwordParams {
  ...
  const thicknessPts = sanitizeProfilePoints(b.thicknessProfile?.points, (t) => clamp(t, 0, 1), (v) => clamp(v, 0.05, 5))
  ...
  if (!['none','crisscross','hineri','katate','wire'].includes(handle.wrapStyle as any)) handle.wrapStyle = 'none'
  if (!['none','katana','paired'].includes(handle.menukiPreset as any)) handle.menukiPreset = 'none'
