...
function endTour(completed = false) {
  try { (window as any).__smkHelpEvents = (window as any).__smkHelpEvents || []; (window as any).__smkHelpEvents.push({ t: Date.now(), event: completed ? 'help.tour_completed' : 'help.tour_canceled' }) } catch {}
...
function placeFor(el: HTMLElement | null) {
  ...
  const r = el?.getBoundingClientRect?.() || { left: window.innerWidth*0.5-120, top: window.innerHeight*0.5-40, width: 240, height: 80 }
...
function startTour(def: Step[]) {
  ...
  const prev = document.createElement('button'); prev.textContent = 'Back'; prev.addEventListener('click', () => { i = Math.max(0, i - 1); renderStep() })
  const next = document.createElement('button'); next.textContent = 'Next'; next.addEventListener('click', () => { if (i < steps.length - 1) { i++; renderStep() } else { endTour(true) } })
  const skip = document.createElement('button'); skip.textContent = 'Skip'; skip.addEventListener('click', () => endTour(false))
  actions.appendChild(prev); actions.appendChild(next); actions.appendChild(skip)
  tip.appendChild(actions)
  overlay.appendChild(box)
  overlay.appendChild(tip)
  document.body.appendChild(overlay)
  highlight = box; tooltip = tip
  try { (window as any).__smkHelpEvents = (window as any).__smkHelpEvents || []; (window as any).__smkHelpEvents.push({ t: Date.now(), event: 'help.tour_started' }) } catch {}
  onKey = (e: KeyboardEvent) => {
    if (e.key === 'Escape') { endTour(false); e.preventDefault(); return }
    if (e.key === 'ArrowRight' || e.key.toLowerCase() === 'n' || e.key === 'Enter') { e.preventDefault(); if (i < steps.length - 1) { i++; renderStep() } else { endTour(true) } }
    if (e.key === 'ArrowLeft' || e.key.toLowerCase() === 'b') { e.preventDefault(); i = Math.max(0, i - 1); renderStep() }
  }
...
export function startIntroTour() {
  ...
  const def: Step[] = [
    ...
    { selector: '#sidebar .row', title: 'Controls', body: 'Most controls have a ? icon. Hover labels for micro-tooltips; click ? for details.' },
    { selector: '.toolbar .dropdown', title: 'Export', body: 'Export GLB/OBJ/STL, plus SVG blueprint.' },
    { selector: '.toolbar button:nth-of-type(1)', title: 'Help', body: 'Open Help (Cmd/Ctrl+/). Search with Cmd/Ctrl+K.' },
    { selector: '.toolbar button:nth-of-type(2)', title: 'Explain Mode', body: 'Toggle labels in the viewport; click a label to open Help.' }
  ]
  startTour(def)
}
