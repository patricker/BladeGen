...
describe('createApplyQualityPreset', () => {
  it('applies Medium preset with FX toggles and registry updates', () => {
    ...
    const rstate: any = { aaMode: 'fxaa', shadowMapSize: 2048, qualityPreset: 'Medium', bloomEnabled: false, bloomStrength: 0.6, bloomThreshold: 0.85, bloomRadius: 0.2 }
    const post: any = { outlineEnabled: false, outlineStrength: 2.5, outlineThickness: 1.0, outlineColor: '#ffffff' }
    const registry = { setValue: (...args: any[]) => calls.push(['reg', ...args]) }
    const refresh = () => calls.push(['refresh'])
    const apply = createApplyQualityPreset(render, rstate, post, registry as any, refresh, ['none','fxaa'])
    apply('Medium', true)
