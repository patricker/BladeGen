...
describe('Per-setting geometry effects (representative)', () => {
  ...
  it('Fuller overlay appears when enabled with slots', () => {
    ...
    (variant.blade as any).fullers = [
      {
        side: 'both',
        width: variant.blade.baseWidth * 0.25,
        depth: 0.015,
        inset: 0.008,
        start: 0.08,
        end: 0.85,
  ...
  it('Distal thickness profile reduces tip thickness', () => {
    ...
    tapered.blade.thicknessProfile = {
      points: [
        [0, 1],
        [1, 0.5],
      ],
  ...
  it('Carved fuller reduces thickness within span (local band)', () => {
    ...
    (carved.blade as any).fullers = [
      {
        side: 'both',
        width: carved.blade.baseWidth * 0.25,
        inset: 0.012,
        start: 0.3,
        end: 0.7,
        mode: 'carve',
  ...
  it('Quillon count > 0 adds guard children', () => {
    ...
    const s1 = new SwordGenerator(base);
    const c1 = groupDescendantsCount(
      ((s1 as any).guardGroup as THREE.Group | null) ?? (s1.guardMesh as any)
    );
    const withQ = defaultSwordParams();
    withQ.guard.quillonCount = 2;
    withQ.guard.quillonLength = 0.3;
    const s2 = new SwordGenerator(withQ);
    const c2 = groupDescendantsCount(
      ((s2 as any).guardGroup as THREE.Group | null) ?? (s2.guardMesh as any)
    );
    expect(c2).toBeGreaterThan(c1);
  });
  ...
  it('Pommel size scales pommel bounds', () => {
    ...
    expect(greater(p2.size.x + p2.size.y + p2.size.z, p1.size.x + p1.size.y + p1.size.z)).toBe(
      true
    );
  });
