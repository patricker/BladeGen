...
export function makeValueNoiseTexture(scale = 8, seed = 1337, size = 256) {
  ...
  const hash = (u:number,v:number) => { const n = Math.sin(u*12.9898+v*78.233+s*0.001)*43758.5453; return n - Math.floor(n) }
  const img = ctx.createImageData(size, size)
  for (let y=0; y<size; y++) {
    for (let x=0; x<size; x++) {
      const nx = (x/size) * scale, ny = (y/size) * scale
      let n = 0, amp = 1, freq = 1
      for (let o=0;o<3;o++) { n += hash(Math.floor(nx*freq)+o*7, Math.floor(ny*freq)+o*19) * amp; amp *= 0.5; freq *= 2 }
