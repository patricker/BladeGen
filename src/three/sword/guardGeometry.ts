...
export function buildGuard(
  g: GuardParams,
  ctx: { bladeMesh?: THREE.Mesh | null; handleMesh?: THREE.Mesh | null; makeMaterial: (part: 'guard') => THREE.Material }
  ...
  if (g.style === 'bar') {
    ...
    geo.center()
    const gmat2 = ctx.makeMaterial('guard') as any; gmat2.side = THREE.DoubleSide
    ...
    const gmat3 = ctx.makeMaterial('guard') as any; gmat3.side = THREE.DoubleSide
