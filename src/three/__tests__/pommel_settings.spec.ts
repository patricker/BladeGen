...
describe('Pommel settings affect geometry as expected', () => {
  it('Elongation increases vertical span for orb style', () => {
    const s1 = makeWithPommel((p) => {
      p.pommel.style = 'orb';
      p.pommel.size = 0.16;
      p.pommel.elongation = 1.0;
    });
    const s2 = makeWithPommel((p) => {
      p.pommel.style = 'orb';
      p.pommel.size = 0.16;
      p.pommel.elongation = 1.8;
    });
  ...
  it('Spike style with higher spikeLength increases vertical span', () => {
    const s1 = makeWithPommel((p) => {
      ...
      p.pommel.spikeLength = 0.8;
      p.pommel.size = 0.16;
    });
    const s2 = makeWithPommel((p) => {
      p.pommel.style = 'spike';
      p.pommel.spikeLength = 1.6;
      p.pommel.size = 0.16;
    });
    const b1 = partBounds(s1, 'pommel')!;
    const b2 = partBounds(s2, 'pommel')!;
  ...
  it('ShapeMorph increases X span for orb style', () => {
    const sA = makeWithPommel((p) => {
      p.pommel.style = 'orb';
      p.pommel.shapeMorph = 0.1;
    });
    const sB = makeWithPommel((p) => {
      p.pommel.style = 'orb';
      p.pommel.shapeMorph = 0.9;
    });
  ...
  it('Offsets move pommel center (X/Y)', () => {
    const base = makeWithPommel((p) => {
      p.pommel.offsetX = 0;
      p.pommel.offsetY = 0;
    });
    const off = makeWithPommel((p) => {
      p.pommel.offsetX = 0.05;
      p.pommel.offsetY = 0.07;
    });
  ...
  it('facetCount increases vertex count for orb style', () => {
    const sL = makeWithPommel((p) => {
      p.pommel.style = 'orb';
      p.pommel.facetCount = 16;
    });
    const sH = makeWithPommel((p) => {
      p.pommel.style = 'orb';
  ...
  it('Balance (auto) changes size vs fixed with defaults', () => {
    const fixed = makeWithPommel((p) => {
      p.pommel.style = 'orb';
      p.pommel.size = 0.16;
      p.pommel.balance = 0;
    });
    const auto = makeWithPommel((p) => {
      p.pommel.style = 'orb';
      p.pommel.size = 0.16;
      p.pommel.balance = 1;
    });
  ...
  it('Ring inner radius increases X span', () => {
    const s1 = makeWithPommel((p) => {
      p.pommel.style = 'ring';
      (p.pommel as any).ringInnerRadius = 0.05;
      p.pommel.size = 0.16;
    });
    const s2 = makeWithPommel((p) => {
      p.pommel.style = 'ring';
      (p.pommel as any).ringInnerRadius = 0.12;
      p.pommel.size = 0.16;
    });
    const b1 = partBounds(s1, 'pommel')!;
    const b2 = partBounds(s2, 'pommel')!;
  ...
  it('Crown spikes increase vertex count', () => {
    const sL = makeWithPommel((p) => {
      p.pommel.style = 'crown';
      (p.pommel as any).crownSpikes = 6;
      (p.pommel as any).crownSharpness = 0.4;
    });
    const sH = makeWithPommel((p) => {
      p.pommel.style = 'crown';
      (p.pommel as any).crownSpikes = 24;
      (p.pommel as any).crownSharpness = 0.4;
    });
    const vL = vertexCount(sL.pommelMesh);
    const vH = vertexCount(sH.pommelMesh);
  ...
  it('Peen visible adds a child to the pommel (non-ring)', () => {
    const s0 = makeWithPommel((p) => {
      p.pommel.style = 'disk';
      (p.pommel as any).peenVisible = false;
    });
    const s1 = makeWithPommel((p) => {
      p.pommel.style = 'disk';
      (p.pommel as any).peenVisible = true;
      (p.pommel as any).peenSize = 0.03;
      (p.pommel as any).peenShape = 'dome';
    });
