...
export function buildScabbard(blade: BladeParams, scabbard: ScabbardParams): ScabbardBuildResult | null {
  ...
  for (let i = 0; i <= segmentCount; i++) {
    ...
    const cx = scabbard.offsetX + bendOffsetX(blade, clampY, bladeLength) + (wav.width !== 0 ? wav.center : 0)
  ...
  const locketEnd = Math.min(1, ((scabbard.locketOffset + scabbard.locketLength) * bladeLength) / Math.max(1e-6, totalLength))
  const chapeStart = Math.max(0, 1 - (scabbard.chapeLength * bladeLength) / Math.max(1e-6, totalLength))
  ...
  const computeDimensions = (s: number, index: number) => {
    ...
    if (s >= chapeStart) {
      ...
      width *= scale
      thickness *= THREE.MathUtils.lerp(1, Math.max(scabbard.chapeScale, 0.35), THREE.MathUtils.clamp(u, 0, 1))
    }
...
export function buildTassel(
  ...
  const p1 = anchor.clone().add(forward.clone().multiplyScalar(ropeLength * 0.2)).add(side.clone().multiplyScalar(0.4))
  const p2 = anchor.clone().add(side).add(sag)
  const p3 = anchor.clone().add(side.clone().multiplyScalar(0.7)).add(down.clone().multiplyScalar(ropeLength))
  ...
  const tuftQuat = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 1, 0), tuftTarget.normalize())
