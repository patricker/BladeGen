...
export class FxManager {
  ...
  attachToComposer(main: EffectComposer) { this.main = main }
  ...
  private ensureBloom() {
    ...
    ;(this.bloomCompositePass.uniforms as any).tBloom.value = this.bloomComposer.renderTarget2.texture
    this.main?.addPass(this.bloomCompositePass)
  }

  private ensureHeat() {
    if (this.heatHazePass && this.heatMaskRT) return
    this.heatMaskRT = new THREE.WebGLRenderTarget(1,1, { depthBuffer: false, stencilBuffer: false })
  ...
  updateSize(width: number, height: number) {
    try { this.bloomComposer?.setSize(width, height) } catch {}
    try { this.heatMaskRT?.setSize(width, height) } catch {}
  }
  ...
  private darkenNonBloom = (o: THREE.Object3D) => {
    ...
    if ((m as any).isMesh && (this.camera.layers.test(m.layers) || true)) {
      ...
      if (this.BLOOM_LAYER !== undefined) {
        const bloomLayers = new THREE.Layers(); bloomLayers.set(this.BLOOM_LAYER)
  ...
  private restoreNonBloom = (o: THREE.Object3D) => {
    const m = o as THREE.Mesh
    if (this.nonBloomMats.has(m)) { m.material = this.nonBloomMats.get(m)!; this.nonBloomMats.delete(m) }
  }
  ...
  setSelectiveBloom(enabled: boolean, opts?: { strength?: number; threshold?: number; radius?: number; intensity?: number }) {
    ...
    if (this.bloomCompositePass && opts?.intensity !== undefined) (this.bloomCompositePass.uniforms as any).intensity.value = opts.intensity
  }
  markForBloom(obj: THREE.Object3D, enable = true) {
    obj.traverse(o => enable ? o.layers.enable(this.BLOOM_LAYER) : o.layers.disable(this.BLOOM_LAYER))
  }
  ...
  setHeatHaze(enabled: boolean, distortion?: number) {
    ...
    if (distortion !== undefined && this.heatHazePass) (this.heatHazePass.uniforms as any).distortion.value = distortion
  }
  markForHeat(obj: THREE.Object3D, enable = true) {
    obj.traverse(o => enable ? o.layers.enable(this.HEAT_LAYER) : o.layers.disable(this.HEAT_LAYER))
  }
}
