...
describe('JSON Schema validation', () => {
  it('default export payload matches schema', () => {
    ...
    const materials = {
      ...
      scabbard:{ color: '#3a2c1c', metalness: 0.2, roughness: 0.65, clearcoat: 0.05, clearcoatRoughness: 0.7, preset: 'None', bumpEnabled: false, bumpScale: 0.015, bumpNoiseScale: 9, bumpSeed: 1337 },
      tassel:{ color: '#7c3f1d', metalness: 0.05, roughness: 0.8, clearcoat: 0.0, clearcoatRoughness: 0.7, preset: 'None', bumpEnabled: false, bumpScale: 0.01, bumpNoiseScale: 10, bumpSeed: 777, sheen: 0.35, sheenColor: '#d8a273' }
    }
    const render = { exposure: 1.0, bgColor: '#0f1115', bgBrightness: 0.0, ambient: 0.4, keyIntensity: 2.0, keyAz: 40, keyEl: 40, rimIntensity: 0.5, rimAz: -135, rimEl: 20, rimColor: '#ffffff', envMapIntensity: 1.0 }
  ...
  it('accepts thicknessProfile points on blade', () => {
    ...
    ;(model.blade as any).curveProfile = { mode: 'absolute', scale: 1, points: [[0,0],[0.5,0.1],[1,0.2]] }
    ;(model.blade as any).widthProfile = { mode: 'scale', points: [[0,1],[0.5,0.8],[1,1.2]] }
    const materials = {
      blade: { color: '#b9c6ff', metalness: 0.8, roughness: 0.25, clearcoat: 0.0, clearcoatRoughness: 0.5, preset: 'None', bumpEnabled: false, bumpScale: 0.02, bumpNoiseScale: 8, bumpSeed: 1337 },
      guard: { color: '#8892b0', metalness: 0.6, roughness: 0.45, clearcoat: 0.0, clearcoatRoughness: 0.5, preset: 'None', bumpEnabled: false, bumpScale: 0.02, bumpNoiseScale: 8, bumpSeed: 1337 },
      handle:{ color: '#5a6b78', metalness: 0.1, roughness: 0.85, clearcoat: 0.0, clearcoatRoughness: 0.6, preset: 'None', bumpEnabled: false, bumpScale: 0.02, bumpNoiseScale: 8, bumpSeed: 1337 },
  ...
  it('accepts advanced fullers and waviness fields', () => {
    ...
    const model = defaultSwordParams()
    model.blade.waviness = { amplitude: 0.02, frequency: 5, mode: 'both', taper: 1.2, offset: 0.01, phase: Math.PI / 2 }
    model.blade.hollowGrind = { enabled: true, mix: 0.7, depth: 0.5, radius: 0.8, bias: 0.1 }
    model.blade.fullers = [
      { side: 'both', offsetFromSpine: 0, width: 0.05, depth: 0.01, inset: 0.006, start: 0.1, end: 0.7, profile: 'u', mode: 'overlay', taper: 0.2 },
      { side: 'right', offsetFromSpine: 0.04, width: 0.03, depth: 0.015, inset: 0.01, start: 0.2, end: 0.6, profile: 'v', mode: 'carve', taper: 0.1 }
    ] as any
    ...
    ;(model.guard as any).langets = { enabled: true, length: 0.15, width: 0.05, thickness: 0.012, chamfer: 0.1 }
