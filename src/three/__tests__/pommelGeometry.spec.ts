...
describe('pommelGeometry', () => {
  it('builds orb and spike variants with placement below handle', () => {
    ...
    const orb = buildPommel(
      { size: 0.2, elongation: 1, style: 'orb', shapeMorph: 0.2, offsetX: 0, offsetY: 0 },
      { handleMesh: handle, blade: null },
      () => mat()
    );
    const spike = buildPommel(
      {
        size: 0.2,
        elongation: 1,
        style: 'spike',
  ...
  it('creates fishtail pommel with pronounced lateral flare', () => {
    const pommel = buildPommel(
      { size: 0.2, elongation: 1, style: 'fishtail', shapeMorph: 0.4, offsetX: 0, offsetY: 0 },
      { handleMesh: undefined, blade: null },
      () => mat()
    );
