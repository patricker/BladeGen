...
import { SwordGenerator, defaultSwordParams, buildBladeOutlinePoints, bladeOutlineToSVG } from '../SwordGenerator'
  ...
  beforeAll(() => {
    if (typeof (globalThis as any).FileReader === 'undefined') {
      class FR {
        ...
        readAsArrayBuffer(_blob: any) { this.onload && this.onload({ target: { result: new ArrayBuffer(0) } }) }
        readAsDataURL(_blob: any) { this.onload && this.onload({ target: { result: 'data:' } }) }
      }
      ;(globalThis as any).FileReader = FR as any
    }
  })
  const canGLTF = typeof (globalThis as any).document !== 'undefined' && typeof (globalThis as any).Blob !== 'undefined'
