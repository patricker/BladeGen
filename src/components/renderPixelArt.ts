...
export function attachRenderPixelArtPanel(opts: {
  ...
  const doc: Document | null =
    typeof document !== 'undefined' ? document : (section as any).ownerDocument || null;
  ...
  select(
    ...
    (v) => {
      ...
      if (state.mode === 'Pixel Art') render.setRenderMode?.('pixelArt');
      else render.setRenderMode?.('standard');
