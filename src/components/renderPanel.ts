...
export function attachRenderQualityPanel(opts: {
  ...
  slider(
    ...
    -0.0005,
    (v) => {
      render.setShadowBias(v);
    },
    () => {},
    'Shadow acne/peter-panning tweak.'
  );

  select(
    rQual,
    'Tone Mapping',
    ['ACES', 'Reinhard', 'Cineon', 'Linear', 'None'],
    rstate.toneMapping,
    (v) => {
  ...
  slider(
    ...
    3.0,
    0.01,
    rstate.envMapIntensity,
    (v) => {
      rstate.envMapIntensity = v;
      render.setEnvIntensity(v);
    },
    () => {},
    'Environment map intensity (reflections).'
  );
}
