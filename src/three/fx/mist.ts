...
export type MistSpawn = { xMin: number; xMax: number; yMin: number; yMax: number; baseTop: number; tipBottom: number; halfT: number }
...
export function updateMistPositions(
  ...
  for (let i=0;i<n;i++){
    ...
    ;(pos.array as any)[ix+0]=x; (pos.array as any)[ix+1]=y; (pos.array as any)[ix+2]=z; arrays.life[i]=life
  }
...
export function buildMist(
  ...
  const spawn: MistSpawn = { xMin, xMax, yMin, yMax, halfT, baseTop: yMin + (yMax - yMin) * 0.2, tipBottom: yMax - (yMax - yMin) * 0.1 }
  for (let i=0;i<count;i++){
    ...
    pos[i*3+0]=xi; pos[i*3+1]=yi; pos[i*3+2]=zi; life[i] = Math.random()
