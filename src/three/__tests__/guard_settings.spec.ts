...
describe('Guard settings affect geometry as expected', () => {
  it('Basket style builds many child rods vs bar', () => {
    const sBar = makeWithGuard((p) => {
      p.guard.style = 'bar';
      p.guard.width = 1.0;
      p.guard.thickness = 0.2;
    });
    const sBasket = makeWithGuard((p) => {
      (p.guard as any).style = 'basket';
      p.guard.width = 1.0;
      p.guard.thickness = 0.2;
      (p.guard as any).basketRodCount = 12;
    });
  ...
  it('Disk cutouts increase vertex count for disk guard', () => {
    const s0 = makeWithGuard((p) => {
      p.guard.style = 'disk';
      p.guard.width = 1.2;
      p.guard.thickness = 0.15;
      (p.guard as any).cutoutCount = 0;
    });
    const s8 = makeWithGuard((p) => {
      p.guard.style = 'disk';
      p.guard.width = 1.2;
      p.guard.thickness = 0.15;
      (p.guard as any).cutoutCount = 8;
      (p.guard as any).cutoutRadius = 0.4;
      p.guard.curveSegments = 24;
    });
  ...
  it('Extras (side rings) add child meshes', () => {
    const sBase = makeWithGuard((p) => {
      p.guard.style = 'bar';
      (p.guard as any).extras = [];
    });
    const sRings = makeWithGuard((p) => {
      p.guard.style = 'bar';
      (p.guard as any).extras = [{ kind: 'sideRing', radius: 0.08, thickness: 0.03, offsetY: 0 }];
    });
  ...
  it('Pas dՉne rings increase child meshes', () => {
    const s0 = makeWithGuard((p) => {
      p.guard.style = 'bar';
      (p.guard as any).pasDaneCount = 0;
    });
    const s2 = makeWithGuard((p) => {
      p.guard.style = 'bar';
      (p.guard as any).pasDaneCount = 2;
      (p.guard as any).pasDaneRadius = 0.05;
      (p.guard as any).pasDaneThickness = 0.01;
    });
  ...
  it('Langets enabled adds two meshes hugging blade', () => {
    const s0 = makeWithGuard((p) => {
      p.guard.style = 'bar';
      (p.guard as any).langets = { enabled: false };
    });
    const s1 = makeWithGuard((p) => {
      p.guard.style = 'bar';
      (p.guard as any).langets = { enabled: true, length: 0.12, width: 0.04, thickness: 0.01 };
    });
    const c0 = groupDescendantsCount((s0 as any).guardGroup ?? s0.guardMesh);
  ...
  it('Winged style curve increases vertical span', () => {
    const s0 = makeWithGuard((p) => {
      ...
      p.guard.curve = 0.0;
      p.guard.tipSharpness = 0.4;
    });
    const sC = makeWithGuard((p) => {
      p.guard.style = 'winged';
      p.guard.curve = 0.7;
      p.guard.tipSharpness = 0.4;
    });
  ...
  it('Higher curveSegments increases vertex count for extruded winged guard', () => {
    const s12 = makeWithGuard((p) => {
      p.guard.style = 'winged';
      p.guard.curveSegments = 8;
    });
    const s48 = makeWithGuard((p) => {
      p.guard.style = 'winged';
      p.guard.curveSegments = 32;
    });
