...
function makeStubContext(geom: THREE.BufferGeometry) {
  ...
  const sword = {
    ...
    setMaterials: (next: Record<string, any>) => { Object.assign(materials, next) },
  ...
  const ctx: RenderHookContext = {
    ...
    outline: { enabled: false, selectedObjects: [], visibleEdgeColor: new THREE.Color(), hiddenEdgeColor: new THREE.Color() } as any,
    ...
    fx: { setSelectiveBloom: noop, setHeatHaze: noop, markForBloom: noop, markForHeat: noop } as any,
    ...
    mistState: {} as any,
    rebuildMist: noop,
    background: { base: new THREE.Color(), target: new THREE.Color(), getBrightness: () => 0, setBrightness: noop, apply: noop, groundMaterial: new THREE.MeshStandardMaterial() },
