...
const resolveFullers = (b: BladeParams): ResolvedFuller[] => {
  ...
  for (const slot of slots) {
    ...
    const profile: ResolvedFuller['profile'] =
      slot.profile === 'v' || slot.profile === 'flat' ? slot.profile : 'u';
...
export function buildBladeGeometry(b: BladeParams): THREE.BufferGeometry {
  ...
  const shapeFactor = (u: number) => {
    ...
    if (cs === 'compound') {
      const primary = Math.pow(
        Math.max(0, 1 - Math.pow(au, THREE.MathUtils.lerp(1.0, 1.8, bevel))),
        1.1
      );
      const shoulderWidth = THREE.MathUtils.lerp(0.6, 0.35, bevel);
      const shoulder = Math.pow(
        Math.max(0, 1 - Math.pow(au / Math.max(0.01, shoulderWidth), 4.0)),
        1.8
      );
  ...
  const serr = (t: number, freq: number, amp: number, bias: number, seedOffset = 0) =>
    serrationWave(t, freq, amp, serPat, serSeed + seedOffset, sharp, bias);
  ...
  const carveFullers = resolvedFullers.filter(
    (f) => f.mode === 'carve' && f.inset > 0 && f.end > f.start
  );
  ...
  for (let i = 0; i <= segs; i++) {
    ...
    const serrL =
      ricassoFrac > 0 && t <= ricassoFrac ? 0 : serr(t, serrFreq, serrAmpL, biasLeft) * (1 - t);
    const serrR =
      ricassoFrac > 0 && t <= ricassoFrac ? 0 : serr(t, serrFreq, serrAmpR, biasRight, 7) * (1 - t);
    ...
    if (b.tipShape === 'clip' || b.tipShape === 'tanto' || b.tipShape === 'sheepsfoot') {
      ...
      if (b.tipShape === 'clip') {
        const delta = baseHalf * 0.4 * Math.pow(tipU, 0.7);
        ...
        const delta = baseHalf * 0.5 * Math.pow(tipU, 2.0);
        if (spineIsRight) rightHalf = Math.max(0.0005, rightHalf - delta);
        else leftHalf = Math.max(0.0005, leftHalf - delta);
    ...
    for (let j = 0; j <= cols; j++) {
      ...
      if (b.hollowGrind && (b.hollowGrind.enabled || (b.hollowGrind.mix ?? 0) > 0)) {
        const hgMix = THREE.MathUtils.clamp(
          b.hollowGrind.mix ?? (b.hollowGrind.enabled ? 0.65 : 0),
          0,
          1
        );
  ...
  for (let i = 0; i < segs; i++) {
    for (let j = 0; j < cols; j++) {
      const f00 = vIndex(i, j, 0),
        f01 = vIndex(i, j + 1, 0);
      const f10 = vIndex(i + 1, j, 0),
        f11 = vIndex(i + 1, j + 1, 0);
      const b00 = vIndex(i, j, 1),
        b01 = vIndex(i, j + 1, 1);
      const b10 = vIndex(i + 1, j, 1),
        b11 = vIndex(i + 1, j + 1, 1);
      indices.push(f00, f01, f11, f00, f11, f10);
      indices.push(b00, b11, b01, b00, b10, b11);
    }
    const lf0 = vIndex(i, 0, 0),
      lb0 = vIndex(i, 0, 1);
    const lf1 = vIndex(i + 1, 0, 0),
      lb1 = vIndex(i + 1, 0, 1);
    indices.push(lf0, lb0, lb1, lf0, lb1, lf1);
    const rf0 = vIndex(i, cols, 0),
      rb0 = vIndex(i, cols, 1);
    const rf1 = vIndex(i + 1, cols, 0),
      rb1 = vIndex(i + 1, cols, 1);
    indices.push(rf0, rb1, rb0, rf0, rf1, rb1);
  }

  for (let j = 0; j < cols; j++) {
    const f0 = vIndex(0, j, 0),
      b0 = vIndex(0, j, 1);
    const f1 = vIndex(0, j + 1, 0),
      b1 = vIndex(0, j + 1, 1);
    indices.push(f0, b1, b0, f0, f1, b1);
    const fn0 = vIndex(segs, j, 0),
      bn0 = vIndex(segs, j, 1);
    const fn1 = vIndex(segs, j + 1, 0),
      bn1 = vIndex(segs, j + 1, 1);
  ...
  const buildRibbon = (slot: ResolvedFuller, faceZ: number) => {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      const halfSlot = Math.min(
        availableHalf * 0.95,
        Math.max(0.002, slot.width * 0.5 * widthScale)
      );
      const cx = bend + slot.offset;
...
export function buildHamonOverlays(b: BladeParams): THREE.Group {
  ...
  const buildEdgeRibbon = (side: 'left' | 'right', z: number) => {
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      const serrX =
        serr > 0 && serrFreq > 0 ? Math.sin(t * Math.PI * serrFreq) * serr * (1 - t) : 0;
  ...
  const thinnerRight =
    (b.thicknessRight ?? b.thickness ?? 0.08) < (b.thicknessLeft ?? b.thickness ?? 0.08);
  const autoSide: 'left' | 'right' = thinnerRight ? 'right' : 'left';

  const wantLeft =
    sidePref === 'left' ||
    sidePref === 'both' ||
    (sidePref === 'auto' && (!single || autoSide === 'left'));
  const wantRight =
    sidePref === 'right' ||
    sidePref === 'both' ||
    (sidePref === 'auto' && (!single || autoSide === 'right'));
...
export function buildBladeOutlinePoints(b: BladeParams): THREE.Vector2[] {
  ...
  const serr = (t: number, freq: number, amp: number, bias: number, offset = 0) =>
    serrationWave(t, freq, amp, serPat, serSeed + offset, sharpOutline, bias);
