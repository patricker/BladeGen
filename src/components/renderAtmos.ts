...
export function attachRenderAtmosPanel(opts: {
  ...
  textRow(gEnv, 'EnvMap URL', atmosState.envUrl, (v) => {
  ...
  select(gEnv, 'Env Preset', ['None','Room','Royal Esplanade','Venice Sunset'], atmosState.envPreset, (v) => {
    applyEnvPreset(v as AtmosState['envPreset'], true)
  }, () => {}, 'Quick env presets (loads remote HDR).')
  checkbox(gEnv, 'Env as Background', atmosState.envAsBackground, (v) => {
  ...
  colorPicker(gFog, 'Fog Color', atmosState.fogColor, (hex) => {
    atmosState.fogColor = hex
    render.setFog?.(hexToInt(atmosState.fogColor), atmosState.fogDensity)
  }, () => {}, 'Fog base color (exp2).')
  slider(gFog, 'Fog Density', 0, 0.1, 0.001, atmosState.fogDensity, (v) => {
  ...
  checkbox(gFresnel, 'Fresnel', atmosState.fresnelEnabled, (v) => {
    atmosState.fresnelEnabled = v
    applyFresnel()
  }, () => {}, 'Additive edge accent based on view angle.')
  slider(gFresnel, 'Fresnel Intensity', 0, 2.0, 0.01, atmosState.fresnelIntensity, (v) => {
    atmosState.fresnelIntensity = v
    applyFresnel()
  }, () => {}, 'Fresnel intensity.')
  slider(gFresnel, 'Fresnel Power', 0.5, 6.0, 0.1, atmosState.fresnelPower, (v) => {
    atmosState.fresnelPower = v
    applyFresnel()
  }, () => {}, 'Fresnel power exponent.')
  colorPicker(gFresnel, 'Fresnel Color', atmosState.fresnelColor, (hex) => {
  ...
  checkbox(gVis, 'Blade Invisible', atmosState.bladeInvisible, (v) => {
    atmosState.bladeInvisible = v
    applyBladeVisibility()
  }, () => {}, 'Hide blade surface but keep effects like aura, glow, mist.')
  checkbox(gVis, 'Occlude When Invisible', atmosState.occludeInvisible, (v) => {
