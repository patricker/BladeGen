...
function buildIndex(docs) {
  ...
  for (const d of docs) {
    ...
    // light synonyms
    if (d.id.includes('fuller')) { terms.add('rib'); terms.add('blood'); terms.add('groove') }
...
try {
  ...
  console.log(`[help-index] Wrote ${Object.keys(index).length} terms for ${docs.length} docs -> ${out}`)
} catch (err) {
