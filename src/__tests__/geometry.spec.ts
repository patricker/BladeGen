...
describe('Blade geometry knobs', () => {
  ...
  it('baseWidth increases base cross-section width', () => {
    const s0 = make((p) => {
      p.blade.baseWidth = 0.2;
      p.blade.tipWidth = 0.05;
    });
    const s1 = make((p) => {
      p.blade.baseWidth = 0.4;
      p.blade.tipWidth = 0.05;
    });
  ...
  it('tipWidth increases overall X span when larger than base', () => {
    const s0 = make((p) => {
      p.blade.baseWidth = 0.25;
      p.blade.tipWidth = 0.02;
    });
    const s1 = make((p) => {
      p.blade.baseWidth = 0.25;
      p.blade.tipWidth = 0.25;
    });
  ...
  it('thicknessLeft/Right increase Z thickness', () => {
    const s0 = make((p) => {
      p.blade.thicknessLeft = 0.04;
      p.blade.thicknessRight = 0.04;
    });
    const s1 = make((p) => {
      p.blade.thicknessLeft = 0.2;
      p.blade.thicknessRight = 0.2;
    });
  ...
  it('sweepSegments increases vertex/face counts', () => {
    const s0 = make((p) => {
      p.blade.sweepSegments = 32;
    });
    const s1 = make((p) => {
      p.blade.sweepSegments = 256;
    });
    const g0 = s0.bladeMesh!.geometry as THREE.BufferGeometry;
    const g1 = s1.bladeMesh!.geometry as THREE.BufferGeometry;
    expect((g1.getAttribute('position') as THREE.BufferAttribute).count).toBeGreaterThan(
      (g0.getAttribute('position') as THREE.BufferAttribute).count
    );
  ...
  it('twistAngle increases Z span', () => {
    const s0 = make((p) => {
      p.blade.twistAngle = 0;
    });
    const s1 = make((p) => {
      p.blade.twistAngle = Math.PI / 2;
    });
    const z0 = span(bboxOf(s0.bladeMesh), 'z');
    const z1 = span(bboxOf(s1.bladeMesh), 'z');
    expect(z1).toBeGreaterThan(z0);
  ...
  it('leaf tip with bulge is wider at mid', () => {
    const base = (p: SwordParams) => {
      p.blade.length = 3;
      p.blade.baseWidth = 0.2;
      p.blade.tipWidth = 0.1;
    };
    const sPoint = make((p) => {
      base(p);
      p.blade.tipShape = 'pointed';
      p.blade.tipBulge = 0.0;
    });
    const sLeaf = make((p) => {
      base(p);
      p.blade.tipShape = 'leaf';
      p.blade.tipBulge = 0.6;
    });
  ...
  it('curvature and baseAngle bend X at mid', () => {
    const sCurved = make((p) => {
      p.blade.curvature = 0.3;
      p.blade.baseAngle = 0;
    });
    const sAngled = make((p) => {
      p.blade.curvature = 0;
      p.blade.baseAngle = 0.1;
    });
  ...
  it('crossSection + bevel increase thickness mid-width (lenticular > diamond > hex)', () => {
    const base = (p: SwordParams) => {
      p.blade.length = 3;
      p.blade.baseWidth = 0.24;
      p.blade.tipWidth = 0.12;
      p.blade.thicknessLeft = 0.08;
      p.blade.thicknessRight = 0.08;
      (p.blade as any).bevel = 1.0;
      p.blade.curvature = 0;
    };
    const mk = (cs: 'diamond' | 'lenticular' | 'hexagonal') =>
      make((p) => {
        base(p);
    ...
    const d = mk('diamond'),
      l = mk('lenticular'),
      h = mk('hexagonal');
    const zD = thicknessAtHalfWidth(d),
      zL = thicknessAtHalfWidth(l),
      zH = thicknessAtHalfWidth(h);
  ...
  it('soriProfile + soriBias shift bend along length (koshi vs saki)', () => {
    ...
    const base = (p: SwordParams) => {
      p.blade.length = 3;
      p.blade.curvature = curvature;
      (p.blade as any).soriBias = 0.8;
      (p.blade as any).baseAngle = 0;
    };
    const koshi = make((p) => {
      base(p);
    ...
    const saki = make((p) => {
      base(p);
      (p.blade as any).soriProfile = 'saki';
    });
...
describe('Guard knobs', () => {
  ...
  it('disk thickness affects Y span (clamped)', () => {
    const sThin = make((p) => {
      p.guard.style = 'disk';
      p.guard.thickness = 0.01;
      p.guard.width = 0.4;
    });
    const sThick = make((p) => {
      p.guard.style = 'disk';
      p.guard.thickness = 0.5;
      p.guard.width = 0.4;
    });
  ...
  it('winged curveSegments increases polygon count', () => {
    const s0 = make((p) => {
      p.guard.style = 'winged';
      (p.guard as any).curveSegments = 6;
    });
    const s1 = make((p) => {
      p.guard.style = 'winged';
      (p.guard as any).curveSegments = 32;
    });
  ...
  it('disk cutouts increase triangle count', () => {
    const mk = (n: number) =>
      make((p) => {
        ...
        p.guard.width = 0.5;
        p.guard.thickness = 0.1;
        (p.guard as any).cutoutCount = n;
        (p.guard as any).cutoutRadius = 0.5;
      });
    const s0 = mk(0),
      s6 = mk(6);
    const mesh0 = s0.guardMesh!,
      mesh6 = s6.guardMesh!;
    const tri = (m: THREE.Mesh) =>
      m.geometry.getIndex()?.count ??
      (m.geometry.getAttribute('position') as THREE.BufferAttribute).count;
  ...
  it('asymmetricArms scales halves differently', () => {
    const s = make((p) => {
      p.guard.style = 'winged';
      (p.guard as any).asymmetricArms = true;
      (p.guard as any).asymmetry = 0.8;
    });
    const group = (s as any).guardGroup as THREE.Group;
    expect(group).toBeTruthy();
    const scales = group.children.filter((c: any) => (c as any).isMesh).map((m: any) => m.scale.x);
  ...
  it('quillonCount and quillonLength/tipSharpness map to geometry', () => {
    const mk = (qc: number, ql: number, ts: number) =>
      make((p) => {
        p.guard.style = 'winged';
        (p.guard as any).quillonCount = qc;
        (p.guard as any).quillonLength = ql;
        (p.guard as any).tipSharpness = ts;
        p.pommel.style = 'orb';
      });
    ...
    s2.group.traverse((o) => {
      const m = o as any;
      if (m.isMesh && (m.geometry as any).type === 'CylinderGeometry')
        cylHeight = Math.max(cylHeight, (m.geometry as any).parameters?.height ?? 0);
      if (m.isMesh && (m.geometry as any).type === 'ConeGeometry')
        coneHeight = Math.max(coneHeight, (m.geometry as any).parameters?.height ?? 0);
    });
  ...
  it('swept style creates multiple guard bars', () => {
    const s = make((p) => {
      (p.guard as any).style = 'swept';
      p.guard.width = 1.2;
      p.guard.thickness = 0.2;
      (p.guard as any).ornamentation = 0.8;
    });
  ...
  it('basket style creates a radial cage', () => {
    const s = make((p) => {
      (p.guard as any).style = 'basket';
      p.guard.width = 1.2;
      p.guard.thickness = 0.2;
      (p.guard as any).ornamentation = 0.5;
    });
    const grp = (s as any).guardGroup as THREE.Group;
  ...
  it('guard extras: loops add torus meshes', () => {
    const s = make((p) => {
      (p.guard as any).extras = [{ kind: 'loop', radius: 0.1, thickness: 0.02, offsetY: 0 }];
    });
    const grp = (s as any).guardGroup as THREE.Group;
    expect(grp).toBeTruthy();
    let torusCount = 0;
    grp.traverse((o) => {
      const m = o as any;
      if (m.isMesh && (m.geometry as any)?.type === 'TorusGeometry') torusCount++;
    });
...
describe('Handle knobs', () => {
  ...
  it('oval ratio stretches X and squashes Z', () => {
    const s1 = make((p) => {
      p.handle.ovalRatio = 1.0;
    });
    const s2 = make((p) => {
      p.handle.ovalRatio = 1.6;
    });
  ...
  it('wrap depth increases X span', () => {
    const s0 = make((p) => {
      ...
      p.handle.wrapDepth = 0;
    });
    const s1 = make((p) => {
      p.handle.wrapEnabled = true;
      p.handle.wrapDepth = 0.03;
  ...
  it('phiSegments increases vertex count', () => {
    const s0 = make((p) => {
      (p.handle as any).phiSegments = 12;
    });
    const s1 = make((p) => {
      (p.handle as any).phiSegments = 96;
    });
    const v0 = (
      (s0.handleMesh!.geometry as THREE.BufferGeometry).getAttribute(
        'position'
      ) as THREE.BufferAttribute
    ).count;
    const v1 = (
      (s1.handleMesh!.geometry as THREE.BufferGeometry).getAttribute(
        'position'
      ) as THREE.BufferAttribute
    ).count;
  ...
  it('curvature bends X around mid section', () => {
    const s0 = make((p) => {
      p.handle.curvature = 0;
    });
    const s1 = make((p) => {
      p.handle.curvature = 0.15;
    });
...
describe('Pommel knobs', () => {
  it('disk is flat along Y regardless of elongation', () => {
    const s1 = make((p) => {
      p.pommel.style = 'disk';
      p.pommel.size = 0.18;
      p.pommel.elongation = 0.5;
    });
    const s2 = make((p) => {
      p.pommel.style = 'disk';
      p.pommel.size = 0.18;
      p.pommel.elongation = 2.0;
    });
  ...
  it('orb elongation scales Y', () => {
    const s1 = make((p) => {
      p.pommel.style = 'orb';
      p.pommel.size = 0.16;
      p.pommel.elongation = 0.8;
    });
    const s2 = make((p) => {
      p.pommel.style = 'orb';
      p.pommel.size = 0.16;
      p.pommel.elongation = 1.6;
    });
    const y1 = span(bboxOf(s1.pommelMesh), 'y');
    const y2 = span(bboxOf(s2.pommelMesh), 'y');
  ...
  it('facetCount increases vertex count', () => {
    const s0 = make((p) => {
      p.pommel.style = 'orb';
      p.pommel.facetCount = 8;
    });
    const s1 = make((p) => {
      p.pommel.style = 'orb';
      p.pommel.facetCount = 64;
    });
    const v0 = (
      (s0.pommelMesh!.geometry as THREE.BufferGeometry).getAttribute(
        'position'
      ) as THREE.BufferAttribute
    ).count;
    const v1 = (
      (s1.pommelMesh!.geometry as THREE.BufferGeometry).getAttribute(
        'position'
      ) as THREE.BufferAttribute
    ).count;
    expect(v1).toBeGreaterThan(v0);
  ...
  it('balance scales size with blade mass', () => {
    ...
    const sLight = make((p) => {
      p.pommel.balance = 1.0;
      p.blade.length = 2.0;
      p.blade.baseWidth = 0.15;
      p.blade.tipWidth = 0.05;
    });
    const sHeavy = make((p) => {
      p.pommel.balance = 1.0;
...
describe('Dependent knobs', () => {
  it('hamon side selection respects side knobs', () => {
    // left only
    const sLeft = make((p) => {
      p.blade.hamonEnabled = true;
      p.blade.hamonWidth = 0.02;
      p.blade.hamonAmplitude = 0.006;
      (p.blade as any).hamonSide = 'left';
    });
    const gLeft = (sLeft as any).hamonGroup as THREE.Group;
    expect(gLeft).toBeTruthy();
    expect(gLeft.children.length).toBe(2);
    // both
    const sBoth = make((p) => {
      p.blade.hamonEnabled = true;
      p.blade.hamonWidth = 0.02;
      p.blade.hamonAmplitude = 0.006;
      (p.blade as any).hamonSide = 'both';
    });
  ...
  it('sori profile/bias have no visible effect when curvature = 0', () => {
    const sTorii = make((p) => {
      p.blade.curvature = 0;
      (p.blade as any).soriProfile = 'torii';
      (p.blade as any).soriBias = 0.8;
      (p.blade as any).chaos = 0;
    });
    const sSaki = make((p) => {
  ...
  it('false edge depth does nothing when length is 0', () => {
    const s0 = make((p) => {
      (p.blade as any).falseEdgeLength = 0;
      (p.blade as any).falseEdgeDepth = 0.05;
    });
    const s1 = make((p) => {
      (p.blade as any).falseEdgeLength = 0;
      (p.blade as any).falseEdgeDepth = 0.0;
    });
  ...
  it('fuller overlay requires an active slot with depth', () => {
    const sNo = make((p) => {
      delete (p.blade as any).fullers;
      p.blade.fullerEnabled = false;
    });
    const sYes = make((p) => {
      (p.blade as any).fullers = [
        {
          side: 'both',
          offsetFromSpine: 0,
          width: 0.05,
  ...
  it('fuller carve reduces Z thickness near mid-face when inset > 0', () => {
    const sFlat = make((p) => {
      ...
      (p.blade as any).fullerInset = 0.0;
      (p.blade as any).fullerMode = 'carve';
    });
    const sCarved = make((p) => {
      p.blade.fullerEnabled = true;
      p.blade.fullerLength = 0.6;
      (p.blade as any).fullerInset = 0.04;
      (p.blade as any).fullerMode = 'carve';
    });
...
describe('Dynamics', () => {
  it('exposes derived metrics in reasonable ranges', () => {
    const s = make((p) => {
      p.blade.length = 3;
      (p.blade as any).thicknessProfile = {
        points: [
          [0, 1],
          [1, 1],
        ],
