...
export function setHighlighter(cb: Highlighter | null) {
  highlighter = cb;
}
export function addDoc(doc: HelpDoc) {
  docs.set(doc.id, doc);
}
export function addDocs(list: HelpDoc[]) {
...
function synthesizeDoc(id: string): HelpDoc | undefined {
  ...
  if (m) {
    ...
    if (kind === 'width') {
      return {
        id,
        label: `${side} F${idx} Width`,
        summary: 'Groove width on this face (scene units).',
        details: ['Wider grooves lighten the blade visually.', 'Keep within the face boundary.'],
        parts: ['blade'],
      };
    } else if (kind === 'offset') {
      return {
        id,
        label: `${side} F${idx} Offset`,
        summary: 'Lateral offset from spine (− left, + right).',
        details: [
          'Use modest offsets to avoid clipping.',
          'Negative toward left (−X), positive toward right (+X).',
        ],
        parts: ['blade'],
      };
    } else if (kind === 'taper') {
      return {
        id,
        label: `${side} F${idx} Taper`,
        summary: 'Taper the groove width toward the tip.',
        details: ['0 flat; 1 narrows along length.'],
        parts: ['blade'],
      };
    }
  }
  // Guard extras: side rings and loops
  if (id === 'guard.side-rings')
    return {
      id,
      label: 'Side Rings',
      summary: 'Enable circular side rings on the guard.',
      details: ['Adds rings to both sides of the guard arms.'],
      parts: ['guard'],
    };
  if (id === 'guard.ring-radius')
    return {
      id,
      label: 'Ring Radius',
      summary: 'Radius of the side ring.',
      details: ['Controls ring size relative to guard.'],
      parts: ['guard'],
    };
  if (id === 'guard.ring-thick')
    return {
      id,
      label: 'Ring Thick',
      summary: 'Tube thickness of the side ring.',
      details: ['Higher values look heavier/robust.'],
      parts: ['guard'],
    };
  if (id === 'guard.ring-offsety')
    return {
      id,
      label: 'Ring OffsetY',
      summary: 'Vertical offset along the blade.',
      details: ['Moves rings up/down along the blade axis.'],
      parts: ['guard'],
    };
  if (id === 'guard.loops')
    return {
      id,
      label: 'Loops',
      summary: 'Enable wire loops on the guard.',
      details: ['Loops sit nearer the blade than side rings.'],
      parts: ['guard'],
    };
  if (id === 'guard.loop-radius')
    return {
      id,
      label: 'Loop Radius',
      summary: 'Radius of the loop.',
      details: ['Controls loop size relative to guard.'],
      parts: ['guard'],
    };
  if (id === 'guard.loop-thick')
    return {
      id,
      label: 'Loop Thick',
      summary: 'Tube thickness of the loop.',
      details: ['Higher values look heavier/robust.'],
      parts: ['guard'],
    };
  if (id === 'guard.loop-offsety')
    return {
      id,
      label: 'Loop OffsetY',
      summary: 'Vertical offset along the blade.',
      details: ['Moves loops up/down along the blade axis.'],
      parts: ['guard'],
    };
  return undefined;
}

export function getDoc(id: string): HelpDoc | undefined {
  return docs.get(id) || synthesizeDoc(id);
}
export function listDocs(): HelpDoc[] {
...
function positionNear(anchor: HTMLElement, el: HTMLElement, prefer: 'below' | 'above' = 'below') {
  ...
  const pad = 6;
  const top =
    prefer === 'below' ? rect.bottom + pad : Math.max(8, rect.top - el.offsetHeight - pad);
...
function destroyPopover() {
  ...
  try {
    (activePopover as any)?.hidePopover?.();
  } catch {}
...
function onOutsideClick(e: MouseEvent) {
  ...
  if (t && activePopover && !activePopover.contains(t) && t !== activePopoverAnchor)
    destroyPopover();
}
...
function findRow(el: HTMLElement | null): HTMLElement | null {
  ...
  while (cur) {
    if (cur.classList?.contains('row')) return cur;
...
function simpleTryThis(row: HTMLElement | null) {
  ...
  if (range) {
    ...
    const stepSet = (v: string) => {
      range.value = v;
      range.dispatchEvent(new Event('input', { bubbles: true }));
      number && (number.value = v);
    };
    stepSet(min);
...
function openPopover(anchor: HTMLElement, doc: HelpDoc, labelText?: string) {
  ...
  if (doc.details && doc.details.length) {
    ...
    for (const d of doc.details.slice(0, 6)) {
  ...
  panelBtn.addEventListener('click', async () => {
    try {
      ...
      try {
        location.hash = `help=${encodeURIComponent(doc.id)}`;
      } catch {}
    }
  });
  ...
  try {
    (pop as any).setAttribute?.('popover', 'manual');
    (pop as any).showPopover?.();
  } catch {}
  // Focus management & optional focus trap when interactive
  const focusableSel =
    'a[href],button:not([disabled]),textarea,select,input:not([disabled]),[tabindex]:not([tabindex="-1"])';
...
export function attachHelp(
  ...
  labelEl.addEventListener('keydown', (e) => {
    if ((e as KeyboardEvent).key === 'Escape') destroyTooltip();
  });
  ...
  if (kbdTarget) {
    kbdTarget.addEventListener('focus', () => {
      if (summary) showTooltip(labelEl, summary);
    });
    kbdTarget.addEventListener('blur', () => {
      destroyTooltip();
    });
  }
  ...
  if (helpIconEl) {
    helpIconEl.addEventListener('click', (e) => {
      ...
      const d =
        ...
        ({
          id: helpId,
          label: labelEl.textContent || helpId,
          summary: summary || '',
          details: [],
        } as HelpDoc);
    ...
    helpIconEl.addEventListener('keydown', (e) => {
      ...
      if (ke.key === 'Enter' || ke.key === ' ') {
        ke.preventDefault();
        (helpIconEl as HTMLElement).click();
      }
    });
  }
}
...
try {
  ...
  /* ignore dynamic import issues in SSR/tests */
}
