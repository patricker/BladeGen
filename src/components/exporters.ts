...
export async function exportGLB(sword: SwordGenerator, variants: MaterialVariant[]): Promise<void> {
  ...
  if (variants && variants.length) {
    ...
    for (const variant of variants) {
      ...
      if (mappings.length) configs.push({ name: variant.name, description: variant.description, mappings })
    }
    if (configs.length) exporter.register((writer) => new KHRMaterialsVariantsExporter(writer, configs, ownedMaterials))
  }
...
export function exportJSON(
  ...
  const payload = { $schema: 'schema/sword.schema.json', version: 4, model: state, render: { ...renderState }, materials: materialsOut } as const
