...
describe('handleGeometry', () => {
  ...
  it('adds wrap style geometry when hineri selected', () => {
    ...
    const tubeCount = handleGroup.children.filter((child) => child instanceof THREE.Mesh && child !== handleGroup.children[0] && (child as THREE.Mesh).geometry instanceof THREE.TubeGeometry).length
    expect(tubeCount).toBeGreaterThan(0)
  })

  it('adds rayskin overlay when enabled', () => {
    const h:any = { length: 1.0, radiusTop: 0.12, radiusBottom: 0.1, rayskin: { enabled: true, scale: 0.01, intensity: 0.6 } }
    const { handleGroup } = buildHandle(h, ()=>mat())
    const latheMeshes = handleGroup.children.filter((child) => child instanceof THREE.Mesh && (child as THREE.Mesh).geometry instanceof THREE.LatheGeometry)
