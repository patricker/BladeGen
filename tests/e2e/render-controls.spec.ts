...
test.describe('Render controls', () => {
  test('toggles AA and bloom without runtime errors', async ({ page }) => {
    ...
    const postFxCheckbox = page.locator(
      '[data-field="render-quality-exposure.post-fx-pipeline"] input[type="checkbox"]'
    );
    ...
    const aaOptions = await aaSelect.evaluate((el) =>
    ...
    const calls = await page.evaluate(
      () => (window as any).__renderHookCalls as Array<{ name: string; args: unknown[] }>
    );
