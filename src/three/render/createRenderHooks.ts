...
export type RenderHooks = {
  setPartMaterial: (
    part: 'blade' | 'guard' | 'handle' | 'pommel' | 'scabbard' | 'tassel',
    patch: MaterialPatch
  ) => void;
...
export function createRenderHooks(context: RenderHookContext): RenderHooks {
  ...
  const renderHooks: RenderHooks = {
    setPartMaterial: (part, patch) => {
      ...
      if (part === 'blade') {
        ...
        if (
          !hasTangents &&
          (nextPatch.anisotropy !== undefined || nextPatch.anisotropyRotation !== undefined)
        ) {
          nextPatch = { ...nextPatch };
    ...
    markForBloom: (obj, enable = true) => {
      fx.markForBloom(obj, enable);
    },
    ...
    markForHeat: (obj, enable = true) => {
      fx.markForHeat(obj, enable);
    },
    ...
    setMistTurbulence: (v) => {
      (mistState as any).turbulence = Math.max(0, v);
    },
    setBackgroundColor: (hex) => {
      background.base.setHex(hex);
      background.apply();
    },
    setBackgroundBrightness: (v) => {
      background.setBrightness(v);
      background.apply();
    },
    setBackgroundTargetColor: (hex) => {
      background.target.setHex(hex);
      background.apply();
    },
    setBaseColor: (hex) => {
      background.groundMaterial.color.setHex(hex);
      background.groundMaterial.needsUpdate = true;
    },
    ...
    setBladeMistAdvanced: (cfg) => {
      ...
      if (cfg.sizeMinRatio !== undefined)
        mistState.sizeMinRatio = Math.max(0, Math.min(1, cfg.sizeMinRatio));
    },
  ...
  (renderHooks as any).setPixelArtOptions = (opts: {
    pixelSize?: number;
    ...
    const inkState = (renderHooks as any)._inkOutlineState as
      | { enabled: boolean; thickness: number; color: number }
      | undefined;
    ...
    const ol = (renderHooks as any)._outlineState as
      | { enabled: boolean; strength: number; thickness: number; color: number }
      | undefined;
